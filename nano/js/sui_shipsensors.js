...
;(function () {
    ...
    function injectStyles() {
        ...
        var css = [
            'body { background: #272727 url(uiBackground.png) 50% 0 repeat-x !important; margin: 0; padding: 10px; }',
            '.sensor-shell { display: grid; gap: 10px; }',
            '.sensor-panel { position: relative; margin: 0; padding: 10px 12px; background: #202020; border: 1px solid rgb(117, 117, 117); }',
            '.sensor-panel__title { margin: 0 0 8px; padding-bottom: 4px; color: #e9c183; font-size: 10px; font-weight: 600; letter-spacing: 0.18em; text-transform: uppercase; border-bottom: 1px solid rgba(64,98,138,0.3); }',
            '.sensor-status { display: flex; align-items: center; gap: 10px; padding: 8px 12px; background: linear-gradient(90deg, rgba(64,98,138,0.15), transparent); border-left: 3px solid #40628a; }',
            '.sensor-status__dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; box-shadow: 0 0 6px currentColor; }',
            ...
            '.sensor-status__detail { color: rgba(255,255,255,0.5); font-size: 11px; }',
            '.sensor-row { display: flex; align-items: center; padding: 3px 0; min-height: 22px; }',
            '.sensor-row__label { width: 110px; flex-shrink: 0; color: #e9c183; font-size: 11px; text-transform: uppercase; letter-spacing: 0.06em; }',
            '.sensor-row__value { flex: 1; display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }',
            '.sensor-btn { display: inline-flex; align-items: center; justify-content: center; gap: 4px; min-height: 22px; padding: 0 8px; color: #ffffff; background: #40628a; border: 1px solid #161616; cursor: pointer; font-size: 11px; letter-spacing: 0.04em; text-transform: uppercase; transition: background 0.15s, box-shadow 0.15s; }',
            '.sensor-btn:hover { background: #507aac; box-shadow: 0 0 6px rgba(64,98,138,0.3); }',
            '.sensor-btn--active { background: #2f943c !important; box-shadow: 0 0 6px rgba(47,148,60,0.3); }',
            '.sensor-btn--disabled { background: #999999 !important; border-color: #666666 !important; opacity: 0.5; cursor: default; pointer-events: none; }',
            ...
            '.sensor-contact { display: flex; align-items: center; gap: 8px; padding: 4px 0; border-bottom: 1px solid rgba(64,98,138,0.15); }',
            '.sensor-contact__dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; box-shadow: 0 0 4px currentColor; }',
            '.sensor-contact__name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 12px; }',
            '.sensor-contact__bearing { color: #e9c183; font-size: 11px; font-variant-numeric: tabular-nums; }',
            '.sensor-contact__compass { position: relative; width: 16px; height: 16px; flex-shrink: 0; }',
            '.sensor-contact__compass::before { content: ""; position: absolute; inset: 0; border: 1px solid rgba(64,98,138,0.4); border-radius: 50%; }',
            '.sensor-contact__needle { position: absolute; top: 2px; left: 7px; width: 2px; height: 7px; background: #8ba5c4; transform-origin: bottom center; box-shadow: 0 0 3px rgba(64,98,138,0.5); }',
            '.sensor-trace { padding: 4px 0; border-bottom: 1px solid rgba(64,98,138,0.15); }',
            '.sensor-trace__header { display: flex; align-items: center; gap: 6px; margin-bottom: 3px; }',
            '.sensor-trace__id { color: #e9c183; font-size: 11px; font-style: italic; }',
            '.sensor-trace__bearing { color: rgba(255,255,255,0.45); font-size: 10px; }',
            '.sensor-trace__pct { margin-left: auto; color: #8ba5c4; font-size: 10px; }',
            '.sensor-scan { font-size: 11px; line-height: 1.5; color: #ffffff; }',
            '.sensor-scan__header { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; padding-bottom: 4px; border-bottom: 1px solid rgba(64,98,138,0.2); }',
            '.sensor-scan__name { color: #e9c183; font-weight: 700; }',
            '.sensor-scan__loc { color: rgba(255,255,255,0.45); font-size: 10px; }',
            '.sensor-scan__body { padding: 6px 8px; background: rgba(0,0,0,0.3); border: 1px solid #40628a; }',
            '.sensor-scan__divider { height: 1px; margin: 4px 0; background: rgba(64,98,138,0.2); }',
            ...
            '.sensor-subtitle { margin-top: 6px; padding-top: 5px; border-top: 1px solid rgba(64,98,138,0.2); color: #e9c183; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; }',
