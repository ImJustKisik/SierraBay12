 ...
 */
/proc/get_path_to(atom/caller, atom/end, max_distance = 30, min_target_dist, id=null, simulated_only = TRUE, turf/exclude, skip_first=TRUE, diagonal_safety=TRUE)
	var/list/path = list()
...
/datum/pathfind/New(atom/caller, atom/goal, id, max_distance, min_target_dist, simulated_only, avoid, skip_first, diagonal_safety, datum/callback/on_finish)
	src.caller = caller
	end = get_turf(goal)
...
/datum/pathfind/proc/search_step()
	var/atom/movable/calling_atom = caller
	if(QDELETED(calling_atom))
		return FALSE
...
/// Legacy wrapper for backwards compatibility with existing AStar calls
/proc/AStar(start, end, adjacent, dist, max_nodes, max_node_depth = 30, min_target_dist = 0, min_node_dist, id, datum/exclude)
	return get_path_to(start, end, max_node_depth, min_target_dist, id, exclude = exclude)
